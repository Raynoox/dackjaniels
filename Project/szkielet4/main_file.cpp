...
void drawObject2(GLuint vao, int vertexCount){
	//jako parametry pewnie trzeba bedzie tez tekstury wrzucic, jedna albo dwie zaleznie od tego magicznego odbicia
	shaderProgram->use();
...
//zwraca tablice GLuintow z buforami
GLuint * setupVBO2(Model_OBJ model){
	GLuint *buffers = new GLuint[4];
	int vertexCount = model.TotalConnectedPoints / 4;
	buffers[0] = makeBuffer(model.vertexBuffer, vertexCount, sizeof(float)* 4); //[0] - bufVertices
	buffers[1] = makeBuffer(model.colors, vertexCount, sizeof(float)* 4); //[1]- bufColors
	buffers[2] = makeBuffer(model.normals, vertexCount, sizeof(float)* 4); //[2]-bufNormals
	buffers[3] = makeBuffer(model.texCoords, vertexCount, sizeof(float)* 4); //[3]bufTexCoords
...
GLuint setupVAO2(GLuint * buffers){
	GLuint vao;
	glGenVertexArrays(1, &vao);
	glBindVertexArray(vao);

	assignVBOtoAttribute("vertex", buffers[0], 4);
	assignVBOtoAttribute("color", buffers[1], 4);
	assignVBOtoAttribute("normal", buffers[2], 4);
	assignVBOtoAttribute("texCoord", buffers[3], 2);
